...
export async function POST(request: NextRequest) {
  ...
  if (updatedInUploads) {
    return redirectWithNotice(request, returnTo, "Job returned to pending", "success");
  }

  const moved = await moveDoneJobToPending(jobPath);

  if (!moved) {
    return redirectWithNotice(request, returnTo, "Failed to return job", "error");
  }

  return redirectWithNotice(request, returnTo, "Job restored to pending", "success");
}
